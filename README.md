...
Two prediction models are available  (7): a statistics-based model (Scikits Gaussian process regression) and an AI model (Lolo Random Forrest Regression). The Curiosity slider (7) allows you to control the extent to which the model explores new material variants ( More precisely, this value corresponds to the weight of the prediction uncertainty in the utility estimate).  
